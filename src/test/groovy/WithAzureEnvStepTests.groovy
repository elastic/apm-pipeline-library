...
class WithAzureEnvStepTests extends ApmBasePipelineTest {

  @Override
  @Before
  void setUp() throws Exception {
    super.setUp()
    script = loadScript('vars/withAzureEnv.groovy')
    env.WORKSPACE = '/foo'
  }
  ...
  void test_secret_error() throws Exception {
    ...
    printCallStack()
    assertTrue(assertMethodCallContainsPattern('error', 'withAzureEnv: Unable to get credentials from the vault: Error message'))
  }
  ...
  void test_secret_missing_data_in_secret() throws Exception {
    ...
    printCallStack()
    assertTrue(assertMethodCallContainsPattern('error', 'withAzureEnv: was not possible to get the authentication info for the tenant field.'))
