...
class WithClusterEnvStepTests extends ApmBasePipelineTest {
  ...
  void test_secret_error() throws Exception {
    ...
    printCallStack()
    assertTrue(assertMethodCallContainsPattern('error', 'withClusterEnv: Unable to get credentials from the vault: Error message'))
  }
  ...
  void test_secret_missing_data_in_secret() throws Exception {
    ...
    printCallStack()
    assertTrue(assertMethodCallContainsPattern('error', 'withClusterEnv: was not possible to get the authentication info for the url field.'))
