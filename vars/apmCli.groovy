...
def call(Map args = [:]) {
  ...
  def reqPy = libraryResource("scripts/apm-cli/requirements.txt")
  def apmCliPy = libraryResource("scripts/apm-cli/apm-cli.py")
  def apmMod = libraryResource("scripts/apm-cli/apm/ApmCli.py")
  def apmModInit = libraryResource("scripts/apm-cli/apm/__init__.py")
  def apmSetup = libraryResource("scripts/apm-cli/setup.py")
  def apmMakefile = libraryResource("scripts/apm-cli/Makefile")
  dir('apm-cli'){
    log(level: 'DEBUG', text: "apmCli: Installing APM CLI")
    writeFile file: "requirements.txt", text: reqPy
    writeFile file: "apm-cli.py", text: apmCliPy
    writeFile file: "setup.py", text: apmSetup
    writeFile file: "Makefile", text: apmMakefile
    dir('apm'){
      writeFile file: "ApmCli.py", text: apmMod
      writeFile file: "__init__.py", text: apmModInit
    }
    if(!url && !token){
    ...
    withEnvMask(vars: [
      ...
      sh(script: "make run", label: 'apm-cli')
      if(saveTsID){
