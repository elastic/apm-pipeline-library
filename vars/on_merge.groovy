...
void call(Map args = [:], body){
  ...
  echo "running because of a merge from ${mergedFrom} to ${target_branch}"
  body()
}
...
def get_merged_from(){
  ...
  def allSourceShas = sh(
    script: "git rev-list HEAD --parents -1",
    returnStdout: true
  )?.trim()?.split(" ")?.toList()
  if(allSourceShas == null){
    error "on_merge: unable to get the source branch"
  }
  def sourceShas = allSourceShas.subList(2,allSourceShas.size())
  def branchNames = ["sss"]
...
String get_feature_branch_sha(){
  node{
    unstash "git-info"
    sh(
      script: "git rev-parse \$(git --no-pager log -n1 | grep 'Merge:')",
      returnStdout: true
     )?.split(" ")[2]?.trim()
  }
}